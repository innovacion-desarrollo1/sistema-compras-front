...
export class SupplierRankingTableComponent implements OnInit, OnChanges {
  ...
  getSemaphoreColor(supplier: ProveedorRanking, index: number): string {
    if (supplier.requiere_aprobacion) return SemaphoreHelper.getColor({ cobertura_dias: 10, es_clase_c: true });
    if (index === 0) return 'verde';
    if (supplier.kpi_score < 70) return 'amarillo';
    return 'gris';
  }

  getSemaphoreText(supplier: ProveedorRanking, index: number): string {
    if (supplier.requiere_aprobacion) return SemaphoreHelper.getText({ cobertura_dias: 10, es_clase_c: true });
